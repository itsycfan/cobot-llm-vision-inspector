...
"""
=============================================================================
COBOT CONTROLLER - Mac Compatible Version
=============================================================================
...
class TimingLogger:
    ...
    def __init__(self):
        ...
        log_dir = Path.home() / "Documents" / "CoBot_Logs"
        log_dir.mkdir(parents=True, exist_ok=True)
        
        # Setup logging
        log_filename = log_dir / f'cobot_control_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        logging.basicConfig(
            level=logging.INFO,
    ...
    def start_operation(self, operation_name: str):
        ...
        self.steps = []
        self.logger.info(f"[START] {operation_name}")
    ...
    def end_step(self, step_name: str = None):
        ...
        if self.step_start_time and self.current_step:
            ...
            self.logger.info(f"[COMPLETE] {self.current_step} - Duration: {duration:.2f}s")
    ...
    def end_operation(self, operation_name: str) -> Dict:
        ...
        if self.start_time:
            ...
            self.logger.info(f"[FINISHED] {operation_name} - Total: {total_duration:.2f}s")
            return report
...
class CobotController:
    ...
    def connect(self) -> bool:
        ...
        if self.simulation_mode:
            self.timing_logger.add_message("[SIM] Virtual robot connected")
        ...
        try:
            ...
            self.timing_logger.add_message(f"[CONNECTED] Robot at {ROBOT_IP}:{ROBOT_PORT}")
            self.timing_logger.end_step()
            return True
            
        except Exception as e:
            self.timing_logger.add_message(f"[ERROR] Connection failed: {str(e)}")
            self.is_connected = False
            self.timing_logger.end_step()
            return False
    ...
    def disconnect(self):
        ...
        self.is_connected = False
        self.timing_logger.add_message("[DISCONNECTED] Robot connection closed")
        self.timing_logger.end_step()
    ...
    def move_to_point(self, point_name: str, wait: bool = True) -> bool:
        ...
        if not self.is_connected:
            self.timing_logger.add_message(f"[ERROR] Cannot move to {point_name}: Robot not connected")
            return False
        
        if self.is_busy:
            self.timing_logger.add_message(f"[ERROR] Robot is busy, cannot move to {point_name}")
            return False
        
        if point_name not in self.robot_points:
            self.timing_logger.add_message(f"[ERROR] Unknown point: {point_name}")
            return False
        ...
        try:
            ...
            if self.simulation_mode:
                ...
                self.timing_logger.add_message(f"[OK] Moved to {point_name}")
            
            self.timing_logger.end_step()
            return True
            
        except Exception as e:
            self.timing_logger.add_message(f"[ERROR] Movement failed: {str(e)}")
            self.timing_logger.end_step()
            return False
    ...
    def execute_sequence(self, sequence_name: str = "default") -> bool:
        ...
        if not self.is_connected:
            self.timing_logger.add_message("[ERROR] Robot not connected")
            return False
        ...
        try:
            if sequence_name == "default" or sequence_name == "high_speed":
                ...
                self.timing_logger.add_message(f"[ERROR] Unknown sequence: {sequence_name}")
                return False
            
            report = self.timing_logger.end_operation(f"{sequence_name} sequence")
            self.timing_logger.add_message(f"[OK] Sequence '{sequence_name}' completed successfully")
            return True
            
        except Exception as e:
            self.timing_logger.add_message(f"[ERROR] Sequence execution failed: {str(e)}")
            return False
    ...
    def execute_custom_sequence(self, points: List[str]) -> bool:
        ...
        try:
            ...
            self.timing_logger.add_message("[OK] Custom sequence completed")
            self.timing_logger.end_operation("Custom sequence")
            return True
            
        except Exception as e:
            self.timing_logger.add_message(f"[ERROR] Custom sequence failed: {str(e)}")
            return False
    ...
    def stop_emergency(self):
        ...
        self.timing_logger.add_message("[STOP] EMERGENCY STOP initiated")
        self.is_busy = False
        self.disconnect()
...
def main():
    ...
    print("\n" + "="*70)
    print("COBOT CONTROLLER - Test Mode (Mac Version)")
    print("="*70 + "\n")
    ...
    if controller.connect():
        ...
        if controller.execute_sequence("high_speed"):
            print("\n[OK] Sequence completed successfully")
